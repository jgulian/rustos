...
impl<M: Mutex<PseudoDirectoryCollection> + 'static> Filesystem for PseudoFilesystem<M> {
    fn root(&mut self) -> io::Result<Box<dyn Directory>> {
        Ok(Box::new(PseudoFilesystemLocation(Path::default(), self.0.clone())))
    }

    fn format(_: &mut dyn BlockDevice, _: &mut PartitionEntry, _: usize) -> io::Result<()> where Self: Sized {
        Err(io::Error::new(io::ErrorKind::Unsupported, "can not format a pseudo filesystem"))
    }
}
...
impl<M: Mutex<PseudoDirectoryCollection> + 'static> Directory for PseudoFilesystemLocation<M> {
    fn open_entry(&mut self, name: &str) -> io::Result<Entry> {
        ...
        path.join_str(name)?;
        let entry = self.1.lock(|directory_collection| {
            let directory = directory_collection.find(&path)?.1;
            directory.entry_type(&path)
        }).map_err(|_| io::Error::new(io::ErrorKind::Unsupported, "poisoned lock in pseudo filesystem"))?
            .ok_or(io::Error::new(io::ErrorKind::NotFound, "entry not found in pseudo filesystem"))?;
    ...
    fn create_file(&mut self, name: &str) -> io::Result<()> {
        Err(io::Error::new(io::ErrorKind::Unsupported, "can not mutate a pseudo filesystem"))
    }
    ...
    fn list(&mut self) -> io::Result<Vec<String>> {
        self.1.lock(|directory_collection| {
            directory_collection.list(&self.0)
        }).map_err(|_| io::Error::new(io::ErrorKind::Unsupported, "poisoned lock in pseudo filesystem"))?
    }

    fn metadata(&mut self) -> io::Result<Box<dyn Metadata>> {
        Err(io::Error::new(io::ErrorKind::Unsupported, "metadata unsupported on pseudo filesystem"))
    }
}

impl<M: Mutex<PseudoDirectoryCollection>> Seek for PseudoFilesystemLocation<M> {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        Err(io::Error::new(io::ErrorKind::Unsupported, "seek unsupported on pseudo filesystem"))
    }
}
...
impl<M: Mutex<PseudoDirectoryCollection>> Read for PseudoFilesystemLocation<M> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.1.lock(|directory_collection| {
            let directory = directory_collection.find(&self.0)
                .ok_or(io::Error::new(io::ErrorKind::NotFound, "entry not found in pseudo filesystem"))?.1;
...
impl<M: Mutex<PseudoDirectoryCollection>> Write for PseudoFilesystemLocation<M> {
    ...
    fn flush(&mut self) -> io::Result<()> {
        Err(io::Error::new(io::ErrorKind::Unsupported, "flush unsupported on pseudo filesystem"))
    }
}
