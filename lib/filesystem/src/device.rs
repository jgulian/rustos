...
pub fn stream_read<I>(device: &mut dyn BlockDevice, offset: usize, blocks: I, data: &mut [u8]) -> io::Result<usize> where I: Iterator<Item=u64> {
...
pub fn stream_write<I>(device: &mut dyn BlockDevice, offset: usize, blocks: I, data: &[u8]) -> io::Result<usize> where I: Iterator<Item=u64> {
    let block_size = device.block_size();
    let (block_offset, small_offset) = offset_data(offset, block_size);
