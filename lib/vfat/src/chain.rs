...
impl<M: Mutex<VirtualFat>> Chain<M> {
    pub(crate) fn new(virtual_fat: Arc<M>) -> VirtualFatResult<Self> {
        let (cluster, capacity) = virtual_fat
            .lock(|virtual_fat| -> VirtualFatResult<(Cluster, u64)> {
    ...
    pub(crate) fn new_from_cluster(
        virtual_fat: Arc<M>,
        cluster: Cluster,
    ) -> VirtualFatResult<Self> {
        let total_size = virtual_fat
            .lock(|virtual_fat| -> VirtualFatResult<u64> {
    ...
    pub(crate) fn new_from_cluster_with_size(
        virtual_fat: Arc<M>,
        cluster: Cluster,
        total_size: u64,
    ) -> VirtualFatResult<Self> {
        let capacity = virtual_fat
            .lock(|virtual_fat| -> VirtualFatResult<u64> {
                Ok((virtual_fat.block_size() * virtual_fat.fat_chain(cluster)?.len()) as u64)
            })
            .map_err(|_| VirtualFatError::FailedToLockFatMutex)??;
    ...
    fn get_blocks(&self, virtual_fat: &mut VirtualFat) -> io::Result<impl Iterator<Item = u64>> {
        Ok(virtual_fat
            .fat_chain(self.current_cluster)
            .map_err(|_| io::Error::from(io::ErrorKind::Other))?
            .into_iter()
            .map(|cluster| Into::<u32>::into(cluster) as u64))
    }

    fn append_cluster(
        &mut self,
        previous: Cluster,
        virtual_fat: &mut VirtualFat,
    ) -> io::Result<()> {
        self.current_cluster = virtual_fat.get_clear_cluster()?;
        virtual_fat
            .update_fat_entry(previous, Status::Data(self.current_cluster))
            .map_err(|_| io::Error::from(io::ErrorKind::Other))?;
        virtual_fat
            .update_fat_entry(self.current_cluster, Status::new_eoc())
            .map_err(|_| io::Error::from(io::ErrorKind::Other))?;
        ...
        self.virtual_fat
            .lock(|virtual_fat| {
                ...
                loop {
                    ...
                    if round_position <= target_position
                        && target_position < round_position + block_size
                    {
